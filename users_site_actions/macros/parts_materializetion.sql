...
{% macro clickhouse__insert_by_parts(temporary, relation, sql) -%}
    {%- set filter_list = config.get('filter_list', validator=validation.any[list]) -%}
    {% set sql_finel = sql.replace('__filter__', filter_list[0]) %}
    {% set create_table = create_table_or_empty(temporary, relation, sql_finel) %}
    {% if adapter.is_before_version('22.7.1.2484') -%}
        {{ create_table }}
    {%- else %}
        {% call statement('main') %}
